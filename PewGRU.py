#!/usr/bin/env python3
"""GRU model with weather and periodic features."""
...
def prepare_features(raw_series: np.ndarray, horizon: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, MinMaxScaler]:
    counts = raw_series[:, -1].reshape(-1, 1)
    weather = raw_series[:, :4]

    weather_scaler = MinMaxScaler()
    weather_scaled = weather_scaler.fit_transform(weather)

    count_scaler = MinMaxScaler()
    counts_scaled = count_scaler.fit_transform(counts)

    idx = np.arange(raw_series.shape[0])
    hour_of_day = idx % 24
    day_of_week = (idx // 24) % 7
    sin_day = np.sin(2 * np.pi * hour_of_day / 24)
    cos_day = np.cos(2 * np.pi * hour_of_day / 24)
    sin_week = np.sin(2 * np.pi * day_of_week / 7)
    cos_week = np.cos(2 * np.pi * day_of_week / 7)
    periodic = np.stack([sin_day, cos_day, sin_week, cos_week], axis=1)

    feature_matrix = np.concatenate([weather_scaled, counts_scaled, periodic], axis=1)

    features = feature_matrix[:-horizon]
    ...
    sequences = features.reshape(-1, LOOKBACK, feature_dim).astype("float32")
...
class PewGRUModel(nn.Module):
    def __init__(self, input_size: int, hidden_size: int = 32, num_layers: int = 2) -> None:
        super().__init__()
...
def evaluate_pew_gru(
    ...
    method_name = f"PewGRU ({target_mode})"
    horizons = [1, 2, 3]
    ...
    for horizon in horizons:
        ...
        for lot_index, lot_id in enumerate(PARK_TABLE_IDS):
            ...
            sequences, labels_scaled, labels_actual, count_scaler = prepare_features(raw_series, horizon)
            ...
            if train_days.size == 0 or test_days.size == 0:
                print(f"[WARN] PewGRU {lot_id} h{horizon}: insufficient data after split")
                continue
            ...
            input_size = train_seq.size(-1)
            model = PewGRUModel(input_size=input_size).to(DEVICE)
            train_torch_model(
                model,
                train_seq,
                train_targets,
                ...
                desc=f"PewGRU ({target_mode}) {lot_id} h{horizon}",
            )
...
def save_outputs(results: list[Dict[str, object]], output_dir: str) -> None:
    ...
    if accuracies:
        ...
        plt.bar(x, accuracies, width=0.5, color="#DD8452")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="GRU with weather and periodic features.")
    ...
    parser.add_argument("--output-dir", type=str, default="results_pew_gru")
    ...
    checkpoint_path = args.checkpoint or os.path.join(CHECKPOINT_DIR, "pew_gru_checkpoint.json")
    checkpoint = load_checkpoint(checkpoint_path)

    results = evaluate_pew_gru(
